  ...
  module Generators
    class InstallGenerator < Rails::Generators::Base
      ...
      def create_db_files
        ...
        migration_template "migrations/create_backend_users.rb", "#{db_migrate_path}/create_backend_users.rb"
      end

      def create_models
        directory "models/backend", "app/models/backend"
        copy_file "models/application_record.rb", "app/models/application_record.rb", force: true
      end

      def create_fixture_file
        copy_file "test_unit/backend_users.yml", "test/fixtures/backend/users.yml"
      end
      ...
      def add_routes
        route "resource  :password_reset", namespace: :backend
        route "resources :users", namespace: :backend
        route "delete 'sign_out', to: 'sessions#destroy'", namespace: :backend
        route "post   'sign_in',  to: 'sessions#create'", namespace: :backend
        route "get    'sign_in',  to: 'sessions#new'", namespace: :backend
        route "get    '/',        to: 'home#index'", namespace: :backend
      end
